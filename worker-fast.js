self.onmessage = function(_ref) {
  ...
  (context = theCanvas.getContext("2d")),
    ...
    (starSpeed = 10),
    (cWidth = theCanvas.width = width),
    (cHeight = theCanvas.height = height),
    (halfWidth = cWidth / 2),
    (halfHeight = cHeight / 2),
    (depth = 1000),
    (colour = ""), // Convert degrees to radians
    (hfov = (100 * Math.PI) / 180),
    (vfov = (80 * Math.PI) / 180), // Set up the view distance based on the field-of-view (with pythagoras)
  ...
  function moveStars(time) {
    ...
    if (time - start > 30) {
      ...
      for (var i = 0; i < stars.length; i++) {
        ...
        stars[i].protectedy = (stars[i].y * vViewDistance) / stars[i].z; // Transform from screen cordinates to X/Y

        stars[i].protectedx += halfWidth;
        stars[i].protectedy = halfHeight - stars[i].protectedy; //colour

        var shade = Math.floor((1 - stars[i].z / depth) * 255); //Size
